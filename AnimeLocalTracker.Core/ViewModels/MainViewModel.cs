    ...
    IRecipient<DescargaProgresoMensaje>
{
    ...
    public async void Receive(NavegarMensaje_Reproductor message)
    {
        ...
        var viewModel = _serviceProvider.GetRequiredService<IReproductorViewModel>();
        try
        {
        ...
        VistaActual = viewModel as CommunityToolkit.Mvvm.ComponentModel.ObservableObject;
    }
    ...
    partial void OnVistaActualChanged(ObservableObject? oldValue, ObservableObject newValue)
    {
        ...
        if (oldValue is IReproductorViewModel reproductorAnterior && !ReferenceEquals(reproductorAnterior, newValue))
        {
        ...
        if (!esConfirmacion)
        {
            ...
            _ = Task.Run(async () => 
            {
                try
                {
                    ...
                    AnimeLocalTracker.Core.Services.CoreDispatcher.Invoke(() => ToastVisible = false);
                }
                catch (Exception ex)
                {
