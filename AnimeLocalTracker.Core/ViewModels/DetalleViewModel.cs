    ...
    public async Task InicializarAsync(AnimeItem anime)
    {
        ...
        var episodiosGenerados = await Task.Run(() => 
        {
            ...
            for (int i = 1; i <= episodiosACargar; i++)
            {
                ...
                {
                    AnimeLocalTracker.Core.Services.AppLogger.Debug("DetalleViewModel", $"Cargado Episodio {ep.NumeroEpisodio}: Progreso={ep.ProgresoSegundos}/{ep.TotalSegundos}, Visto={ep.Visto}, TieneProgreso={ep.TieneProgresoGuardado}");
                }
