...
public class AniListService(HttpClient httpClient) : IAniListService
{
    ...
    public async Task<AniListMedia?> ObtenerAnimePorIdAsync(int id)
    {
        try
        {
            ...
            var response = await cliente.PostAsync("https://graphql.anilist.co", requestContent);
    ...
    public async Task<bool> ActualizarProgresoAsync(int mediaId, int episodio, string token)
    {
        try
        {
            // Instanciamos el cliente HTTP aquí mismo para evitar el error de contexto
            using var cliente = new HttpClient(); 
            ...
            // Ahora usamos la variable "cliente" que acabamos de crear
